...
| [`apps/jevball`](apps/jevball/README.md) | Magic-Jev-Ball: a Jev-backed Magic 8 Ball, built with React, Vite, and Cloudflare Workers. |
...
`bun run dev` starts the personal site on port 4444. `bun run dev:diffly` starts Diffly on port 4445; `bun run dev:jevball` starts Magic-Jev-Ball on port 4446. Each app is developed and deployed independently. `bun run build:web` syncs the vault and builds only the personal site.
